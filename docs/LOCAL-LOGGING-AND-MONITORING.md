...
| A new actionable activation has no controller event. | The running Codex process did not load the installed controller hook, or the main treated the mention as discussion and did not run the bounded activation command. | Start a fresh process, verify `hooks.json`, and confirm the main received the activation-candidate context. |
| A child tries to request permission escalation. | The route violated the no-prompt child contract. | The hook denies it before a user prompt; retry in scope without escalation or return the action to the main. |
